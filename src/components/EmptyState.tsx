/**
 * EmptyState — 빈/완료/오류 상태 공용 컴포넌트 (v5 그린 듀오톤).
 * 스팟 일러스트는 빈 상태 전용(실서비스도 empty state에는 절제된 일러스트 사용) —
 * 콘텐츠 영역에서는 금지(사진이 원칙). 문법: 틴트 블롭 → 흰 면+그린 스트로크 → 테라코타 포인트.
...
const STROKE = { stroke: palette.green600, strokeWidth: 5, strokeLinecap: 'round', strokeLinejoin: 'round' } as const;
...
export function SpotIllustration({ kind, size = 112 }: { kind: SpotKind; size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 120 120">
      {kind === 'compass' && (
        <>
          <Blob fill={palette.greenTint} />
          <Circle cx={60} cy={60} r={26} fill={palette.white} {...STROKE} />
          {/* 나침반 바늘: 테라코타 상단 + 그린 하단 */}
          <Path d="M60,42 L67,60 L60,58 L53,60 Z" fill={palette.terracotta} />
          <Path d="M60,78 L53,60 L60,62 L67,60 Z" fill={palette.green600} fillOpacity={0.35} />
          <Circle cx={60} cy={60} r={3} fill={palette.green600} />
        </>
      )}
      {kind === 'chat' && (
        <>
          ...
          <Circle cx={48} cy={56} r={3.5} fill={palette.terracotta} />
          <Circle cx={60} cy={56} r={3.5} fill={palette.terracotta} />
          <Circle cx={72} cy={56} r={3.5} fill={palette.terracotta} />
        </>
      )}
      {kind === 'heart' && (
        <>
          <Blob fill={palette.terracottaTint} />
          <Circle cx={60} cy={60} r={27} fill={palette.white} {...STROKE} stroke={palette.terracotta} />
          <Path
      ...
      {kind === 'cloud' && (
        <>
          ...
          <Path
            ...
            {...STROKE}
          />
          <Circle cx={48} cy={86} r={3} fill={palette.green600} />
          <Circle cx={62} cy={92} r={3} fill={palette.green600} />
          <Circle cx={76} cy={86} r={3} fill={palette.green600} />
        </>
      )}
    </Svg>
  );
